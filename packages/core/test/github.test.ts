...
`;
  ...
  it("scans a repo end to end and grades skills, agents, and .mcp.json", async () => {
    ...
    // clean skill, deterministic-only → structure passes; VERIFY-03 is informational (a
    // per-skill scan can't see repo-root hygiene), so nothing warns and the grade is A+.
